    ...
    <div class="d-flex align-items-center">
        <div class="mr-auto">
            ...
            <ul class="m-subheader__breadcrumbs m-nav m-nav--inline">
                ...
                <li class="m-nav__item">
                    <a href="{{url('admin/message/new')}}" class="m-nav__link">
											<span class="m-nav__link-text">
												send message
											</span>
                    </a>
		    	...
		    	<div class="m-portlet__head">
					<div class="m-portlet__head-caption">
						<div class="m-portlet__head-title">
							...
							<!--begin::Form-->
							<form class="m-form m-form--fit m-form--label-align-right" method="POST" action="{{ url('admin/message/send-message') }}" enctype="multipart/form-data">
                            {{ csrf_field() }}
								...
								<div class="m-portlet__body">
									<div class="form-group m-form__group row {{ $errors->has('receiver') ? ' has-error' : '' }}">
												...
												<div class="col-lg-9 col-md-9 col-sm-12">
                                                    <select class="form-control m-input" id="receiver" name="receiver">
                                                    <option disabled selected> Select Receiver </option>
                                                        @php
                                                            $users = \App\Http\Helpers::getAllUsers();
                                                        @endphp
                                                        @foreach($users as $user)
                                                            <option value="{{$user->email}}" @if($user->email ==Auth::guard('admin')->user()->email) disabled @endif> {{$user->en_name}} </option>
                                                        @endforeach
									...
									<div class="form-group m-form__group row{{ $errors->has('content') ? ' has-error' : '' }}">
												...
												<div class="col-lg-9 col-md-9 col-sm-12">
												<textarea class="form-control m-input" id="content" name="content" rows="10" value="">{{old('content')}}</textarea>
